  ...
  useEffect(() => {
    const checkGoogleOAuth = async () => {
      try {
        ...
        // Only enable if response is OK (200) and we get a URL back
        if (response.ok && response.status === 200) {
          const data = await response.json();
          // Verify we actually got a URL back
          if (data.url) {
          ...
          // 501 or other errors mean it's not configured
          setGoogleOAuthEnabled(false);
        }
      } catch (err) {
        // Google OAuth not configured or network error - that's fine
        setGoogleOAuthEnabled(false);
      }
    };
    checkGoogleOAuth();
